...
class FlxButton extends FlxTypedButton<FlxText>
{
     ...
     * @param   TextSize  The size of the text that you want to appear on the button.
	 */
	public function new(X:Float = 0, Y:Float = 0, ?Text:String, ?TextSize:Int = 8, ?OnClick:Void->Void)
	{
		super(X, Y, OnClick);
	...
	inline function initLabel(Text:String, Size:Int = 8):Void
	{
		if (Text != null)
		{
			label = new FlxText(x + labelOffsets[NORMAL].x, y + labelOffsets[NORMAL].y, 80, Text);
			label.setFormat(null, Size, 0x333333, "center");

...
/**
 * The TJSON library. The reason it is in the source folder instead of being used as a separate library is
 * because the original TJSON had a bug in its encoder; This bug is fixed in this version.
 */
class TJSON {
		...
		while (pos < json.length) {
			...
			if (key == "}" && !lastSymbolQuoted) {
				...
				if (isClassOb && #if flash9 try o.TJ_unserialize != null catch (e:Dynamic) false #elseif (cs || java) Reflect.hasField(o,
					"TJ_unserialize") #else o.TJ_unserialize != null #end) {
					o.TJ_unserialize();
				}
				return o;
			}
			...
			if (inQuote) {
				if (inEscape) {
					...
					if (c == "u") {
						...
						var utf = new UnicodeString('$hexValue');
						symbol += utf.toString();
		...
		switch (Type.typeof(obj)) {
			case TClass(c):
				...
				if (#if flash9 try
					...
					false #elseif (cs || java) Reflect.hasField(obj, "TJ_noEncode") #else obj.TJ_noEncode != null #end) {

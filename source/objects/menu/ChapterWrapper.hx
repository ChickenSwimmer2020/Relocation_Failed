...
class ChapterSelecterGroup extends FlxSpriteGroup {
	...
	private function createChapterBox(index:Int):ChapterBox {
		return new ChapterBox(startX + index * spacing, 285, // Fixed Y position
			chapterImages[index], chapterNames[index], onChapterClick(index), chapterLocks[index]);
	}
	...
	private function onMouseWheel(event:MouseEvent):Void {
		...
		currentOffset = RFMath.clamp(currentOffset + (if (event.delta < 0) scrollSpeed else -scrollSpeed), minOffset, maxOffset);
	...
	private function onChapterClick(index:Int):Void->Void {
		return function():Void {
			...
			switch (index) {
				case 0: // Chapter 1
					FlxG.switchState(() -> new Playstate('level0')); //* level name passthrough, so we have to use ()->new Playstate

...
class NPC extends FlxGroup {
	...
	**/ // TODO: implement
	public function new(xPos:Float, yPos:Float, FrameBounds:Array<Int>, Spr:String, /*Behavior:NPCBehavior, Weapon:String,*/ FreeMovement:Bool,
			?forcedMovementPath:Array<FlxPoint>) {
		super();
		NPC = new FlxSprite(xPos, yPos).loadGraphic(Assets.image(Spr), true, FrameBounds[0], FrameBounds[1]);
		add(NPC);

		_VIEW = [
			new FlxPoint(xPos, yPos),
			new FlxPoint(xPos + _VIEWRANGE, yPos + FOV),
			new FlxPoint(xPos + -_VIEWRANGE, yPos + -FOV),
			new FlxPoint(xPos, yPos)
		];

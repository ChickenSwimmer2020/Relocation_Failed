...
class IntroState extends FlxState {
	...
	override public function create() {
		...
		ship = new FlxSprite(-120, -30, 'assets/menu/ship-off.png');
		...
		add(overlay);
		var vingette = new FlxSprite(0, 0, 'assets/menu/Vingette.png');
		vingette.alpha = 0.7;
		...
		add(skipTxt);
		planet = new FlxSprite(0, 0, 'assets/menu/planet.png');
		planet.alpha = 0;
		add(planet);
					...
					wait(2, () -> {
						...
						wait(2, () -> {
							...
							wait(2, () -> {
								...
								wait(1, () -> {
									FlxTween.tween(overlayWhite, {alpha: 1});
								});
								wait(2, () -> {
	...
	override public function update(elapsed:Float) {
		...
		if (lerpWindow && fromWindowIntro) {
			val += 0.02;
			var window = Application.current.window;
			window.x = Std.int(LunarInterp.easedInterp(window.x, WindowIntro.oldWindowPosition.x, val, 'smootherStepInOut'));
			window.y = Std.int(LunarInterp.easedInterp(window.y, WindowIntro.oldWindowPosition.y, val, 'smootherStepInOut'));
			window.width = Std.int(LunarInterp.easedInterp(window.width, WindowIntro.oldWindowDimensions.x, val, 'smootherStepInOut'));
			window.height = Std.int(LunarInterp.easedInterp(window.height, WindowIntro.oldWindowDimensions.y, val, 'smootherStepInOut'));
		}
